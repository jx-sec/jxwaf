  ...
  <div class="custom-edit-wrap">
    <el-row class="breadcrumb-style">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/user/web-white-rule' }">Web白名单规则</el-breadcrumb-item>
    ...
    <el-row class="container-style">
      <el-col :span="24" v-loading.fullscreen.lock="loadingPage">
        <el-form
          ...
          :model="webRuleManageForm"
          :rules="rules"
          ref="webRuleManageForm"
          label-width="120px"
        >
          <div>
            <el-form-item label="规则名称" prop="rule_name">
              <el-input
                v-if="uuid == 'new'"
                v-model="webRuleManageForm.rule_name"
            ...
            <el-form-item label="执行动作" prop="rule_action">
              <el-select
                v-model="webRuleManageForm.rule_action"
                placeholder="请选择"
        ...
        <el-row type="flex" class="margin-border" justify="space-between">
          ...
          <el-col :span="12" class="text-align-right">
            <el-button
              type="primary"
              @click="onClickWebRuleProSubmit('webRuleManageForm')"
              :loading="loading"
              >保存
            </el-button>
          </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      ruleAction: [
        { value: 'web_bypass', label: 'Web安全防护加白' },
  ...
  methods: {
    getData() {
      var t = this
      var url = '/user/get_web_white_rule'
    ...
    onClickWebRuleProSubmit(webRuleManageForm) {
      ...
      if (t.uuid == 'new') {
        postUrl = '/user/create_web_white_rule'
      } else {
        postUrl = '/user/edit_web_white_rule'
        t.webRuleManageForm.rule_name = t.uuid
      }
      ...
      this.$refs[webRuleManageForm].validate((valid) => {
        if (valid) {
          ...
          JXAjax(
            ...
            function (response) {
              t.loading = false
              t.$router.push('/user/web-white-rule')
            },
            function () {
              t.loading = false
            }
