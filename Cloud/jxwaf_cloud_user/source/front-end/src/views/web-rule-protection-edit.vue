  ...
  <div class="custom-edit-wrap">
    <el-row class="breadcrumb-style">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/user/web-rule-protection' }">Web规则配置</el-breadcrumb-item>
...
export default {
  ...
  methods: {
    getData() {
      var t = this
      var url = '/user/get_web_rule_protection'
    ...
    onClickWebRuleProSubmit(webRuleManageForm) {
      ...
      if (t.uuid == 'new') {
        postUrl = '/user/create_web_rule_protection'
      } else {
        postUrl = '/user/edit_web_rule_protection'
        t.webRuleManageForm.rule_name = t.uuid
      }
      ...
      this.$refs[webRuleManageForm].validate((valid) => {
        if (valid) {
          ...
          JXAjax(
            ...
            function (response) {
              t.loading = false
              t.$router.push('/user/web-rule-protection')
            },
            function () {
              t.loading = false
            }
