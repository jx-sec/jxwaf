...
**部署架构**  
标准版将 WAF 节点、控制台、MySQL、日志采集、网络封禁整合在一台服务器上部署。专业版将这些组件分离，每层可独立扩展。

**日志能力**  
标准版攻击日志存储于 MySQL，支持基础查询和筛选。专业版基于 ClickHouse，支持千万级日志的秒级聚合分析，配合可视化报表，适合需要安全事件回溯和合规审计的企业。

**扩展性**  
标准版为单节点部署，日均可处理超 8 亿次请求检测。当业务增长需要多节点时，可平滑迁移至专业版，核心引擎完全相同。
